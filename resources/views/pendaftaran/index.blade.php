...
<body>
                        ...
                        <ul class="flex flex-col lg:flex-row gap-8 md:gap-6 items-center pt-14 lg:pt-0">
                            ...
                            <li class="has-child {{ request()->is('pendaftarans') ? 'active' : '' }}">
                                <div class="flex justify-between space-x-44 lg:space-x-0 gap-1 items-center">
                                    <div>
                                        <a class="active" href="/pendaftarans">Pendaftaran</a>
                                    </div>
                                    <div>
                                        <span class="caret">
                                            <svg class="fill-current text-white w-5 h-5"
    ...
    <div class="py-12 h-[50vh]">

        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">

            <x-success-status class="mb-4" :status="session('message')" />

            <div class="overflow-x-auto py-4 px-4 bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">

                <table class="border-collapse w-full border-solid border-2 border-slate-600">
                    <thead>
                        <tr class="text-center bg-primary text-white">
                            <th class="border-solid border-2 border-slate-600">ID</th>
                            <th class="border-solid border-2 border-slate-600">Nama Lengkap</th>
                            <th class="border-solid border-2 border-slate-600">Jenis Kelamin</th>
                            <th class="border-solid border-2 border-slate-600">No.HP</th>
                            <th class="border-solid border-2 border-slate-600">Tanggal Lahir</th>
                            <th class="border-solid border-2 border-slate-600">Tempat Lahir</th>
                            <th class="border-solid border-2 border-slate-600">Kewarganegaraan</th>
                            <th class="border-solid border-2 border-slate-600">NIK</th>
                            <th class="border-solid border-2 border-slate-600">Edit</th>
                            <th class="border-solid border-2 border-slate-600">Delete</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse ($pendaftarans as $pendaftaran)
                            <tr>
                                <td class="text-center border-solid border-2 border-slate-600">{{ $pendaftaran->id }}
                                </td>
                                <td class="border-solid border-2 border-slate-600">{{ $pendaftaran->name }}</td>
                                <td class="text-center border-solid border-2 border-slate-600">
                                    {{ $pendaftaran->gender }}</td>
                                <td class="text-center border-solid border-2 border-slate-600">
                                    {{ $pendaftaran->phone }}</td>
                                <td class="text-center border-solid border-2 border-slate-600">
                                    {{ $pendaftaran->tanggal_lahir }}</td>
                                <td class="text-center border-solid border-2 border-slate-600">
                                    {{ $pendaftaran->tempat_lahir }}</td>
                                <td class="text-center border-solid border-2 border-slate-600">
                                    {{ $pendaftaran->negara }}</td>
                                <td class="text-center border-solid border-2 border-slate-600">{{ $pendaftaran->nik }}
                                </td>
                                <td
                                    class="text-center border-solid border-2 border-slate-600 bg-blue-500 hover:bg-blue-800 text-white py-2 px-2">
                                    <a href="{{ url('/edit-pendaftaran/'.$pendaftaran->id ) }}"
                                        class="btn btn-primary">Edit</a>
                                </td>
                                <td
                                    class="text-center border-solid border-2 border-slate-600 bg-red-500 hover:bg-red-800 text-white py-2 px-2">
                                    <form action="{{ url('delete-pendaftaran/'.$pendaftaran->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
